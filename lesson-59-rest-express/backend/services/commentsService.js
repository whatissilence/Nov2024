...
export const getCommentsByPostId = (postId) => {
  return comments.filter(comm => comm.postId === Number(postId));
}

export const getCommentById = (commId) => {
  return comments.find(comm => comm.id === Number(commId));
}

export const deleteComment = (id) => {
  const commIndex = comments.findIndex(comm => comm.id === Number(id));
  if (commIndex === -1) {
    return;
  }
